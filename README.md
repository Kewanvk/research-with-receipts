...
Run this first to see what you already have and what's missing:

```bash
echo "=== Environment Check ===" && \
echo -n "Claude Code: " && (claude --version 2>/dev/null || echo "NOT FOUND — install from https://docs.anthropic.com/en/docs/claude-code") && \
echo -n "Node.js:     " && (node --version 2>/dev/null || echo "NOT FOUND") && \
echo -n "Python 3:    " && (python3 --version 2>/dev/null || echo "NOT FOUND") && \
echo -n "pipx:        " && (pipx --version 2>/dev/null || echo "NOT FOUND") && \
echo -n "nlm CLI:     " && (nlm --version 2>/dev/null || echo "NOT FOUND") && \
echo -n "python-docx: " && (python3 -c "import docx; print('OK')" 2>/dev/null || echo "NOT FOUND") && \
echo "========================="
```

Everything should show a version number or "OK". Fix any "NOT FOUND" items below.

### Step 1: Install Dependencies

Only install what's missing from Step 0.
...
**Python 3** (>= 3.9):
```bash
# macOS (usually pre-installed, check with python3 --version)
brew install python
...
After installing pipx, **restart your terminal** for PATH changes to take effect.

### Step 2: Install NotebookLM CLI + MCP Server

```bash
# Install the nlm CLI
pipx install notebooklm-mcp-cli

# Register NotebookLM as an MCP server in Claude Code
nlm setup add claude-code

# Install the NotebookLM skill (teaches Claude how to use NotebookLM properly)
nlm skill install claude-code
```

> **Why both `setup add` and `skill install`?** `setup add` registers the connection between Claude Code and NotebookLM. `skill install` gives Claude the full instruction manual — without it, Claude only knows basic operations (create/delete notebooks) but can't do uploads, queries, or content generation.

### Step 3: Authenticate

```bash
# Opens Chrome for Google OAuth login
nlm login
```

You need a Google account. Free tier works (50 queries/day). Sessions expire after ~20 minutes — re-run `nlm login` when commands start failing.
...
Restart Claude Code, then run the environment check from Step 0 again. Everything should pass.

You can also verify NotebookLM is working:
```bash
nlm notebook list
```

If this returns a list (or empty list) without errors, you're good to go.
...
```bash
echo "=== 环境检查 ===" && \
echo -n "Claude Code: " && (claude --version 2>/dev/null || echo "未安装 — 见 https://docs.anthropic.com/en/docs/claude-code") && \
echo -n "Node.js:     " && (node --version 2>/dev/null || echo "未安装") && \
echo -n "Python 3:    " && (python3 --version 2>/dev/null || echo "未安装") && \
echo -n "pipx:        " && (pipx --version 2>/dev/null || echo "未安装") && \
echo -n "nlm CLI:     " && (nlm --version 2>/dev/null || echo "未安装") && \
echo -n "python-docx: " && (python3 -c "import docx; print('OK')" 2>/dev/null || echo "未安装") && \
echo "================="
```

全部显示版本号或 "OK" 就没问题。有 "未安装" 的看下面对应步骤。
...
**Python 3**（>= 3.9）：
```bash
# macOS（通常已预装，用 python3 --version 确认）
brew install python
...
装完 pipx 后**重启终端**，让 PATH 生效。

### 第 2 步：安装 NotebookLM CLI + MCP Server

```bash
# 安装 nlm 命令行工具
pipx install notebooklm-mcp-cli

# 把 NotebookLM 注册为 Claude Code 的 MCP 服务
nlm setup add claude-code

# 安装 NotebookLM 的操作手册（教 Claude 怎么正确使用 NotebookLM）
nlm skill install claude-code
```

> **为什么要两步？** `setup add` 只注册了连接通道。`skill install` 是给 Claude 装操作手册——没有它，Claude 只会基本操作（建/删笔记本），不会上传素材、提问、生成播客等高级功能。

### 第 3 步：登录认证

```bash
# 会打开 Chrome 完成 Google 登录
nlm login
```

需要 Google 账号，免费版即可（每天 50 次查询）。登录态约 20 分钟过期，失效后重新跑 `nlm login`。
...
重启 Claude Code，再跑一遍第 0 步的检查命令，确认全部通过。

也可以验证 NotebookLM 是否正常：
```bash
nlm notebook list
```

如果返回列表（或空列表）且没报错，就说明一切就绪。
